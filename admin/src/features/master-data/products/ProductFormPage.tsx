...
import ProductFormUnsavedModal from '@/features/master-data/products/components/ProductFormUnsavedModal'
...
const ProductFormPage = () => {
  ...
  const handleCreated = (id: string) => {
    form.markTabDirty('product', false)
    showNotification({
      title: 'Thành công',
  ...
  const handleProductDirtyChange = (dirty: boolean) => {
    form.markTabDirty('product', dirty)
  }
  ...
  const footer = (
    <div className="border-top mt-4 pt-3">
      <ProductFormFooter activeTab={form.activeTab} isNew={form.isNew} isSaving={form.isSaving} />
    </div>
  )
  ...
  return (
    <Container fluid>
      ...
      <ProductWizard
        ...
        onStepChange={form.requestTabChange}
        productStep={
          <>
            <ProductInfoTab
              productId={form.productId}
              ...
              isNew={form.isNew}
              resetNonce={form.formResetNonce}
              onCreated={handleCreated}
              onSaved={handleSaved}
              onSavingChange={form.setIsSaving}
              onDirtyChange={handleProductDirtyChange}
              onRegisterSave={form.registerProductSave}
            />
            {footer}
          </>
        }
      ...
      <ProductFormUnsavedModal
        show={form.showUnsavedModal}
        isSaving={form.isSaving}
        onHide={() => form.setShowUnsavedModal(false)}
        onSave={() => void form.confirmUnsavedSave()}
        onDiscard={form.discardPendingTabChange}
      />
    </Container>
  )
}
