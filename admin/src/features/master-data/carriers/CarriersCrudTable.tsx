...
import { CARRIER_PAGE_SIZE_OPTIONS } from '@/features/master-data/carriers/carriers.api'
import { buildCarrierColumns } from '@/features/master-data/carriers/columns'
import { carriersLabels } from '@/features/master-data/carriers/data'
import { useCarriersCrud } from '@/features/master-data/carriers/useCarriersCrud'
import EntityFormModal from '@/modules/crud/form/EntityFormModal'

const CarriersCrudTable = () => {
  const crud = useCarriersCrud({
    buildColumns: buildCarrierColumns,
  })

  const deleteMessage =
    crud.pendingDeleteCount > 1
      ? `Bạn có chắc muốn xóa ${crud.pendingDeleteCount} ${carriersLabels.itemName} đã chọn?`
      : `Bạn có chắc muốn xóa ${carriersLabels.itemName} này?`
  ...
  return (
    <Card>
      <CardHeader className="border-light justify-content-between">
        <div className="d-flex align-items-center gap-2 flex-wrap">
          <div className="app-search">
            <input
              ...
              placeholder={carriersLabels.searchPlaceholder}
        ...
        <div className="card-action d-flex flex-nowrap align-items-center gap-2">
          ...
          <Button
            ...
            {crud.isLoadingLookups ? 'Đang tải...' : carriersLabels.addButton}
          </Button>
        </div>
      </CardHeader>
      ...
      {!crud.isLoading && crud.paginationInfo.total > 0 && (
        <CardFooter className="border-0">
          <TablePagination
            ...
            itemsName={carriersLabels.itemName}
            pageSize={crud.pageSize}
            pageSizeOptions={CARRIER_PAGE_SIZE_OPTIONS}
      ...
      <DeleteConfirmationModal
        ...
        itemName={carriersLabels.itemName}
...
export default CarriersCrudTable
