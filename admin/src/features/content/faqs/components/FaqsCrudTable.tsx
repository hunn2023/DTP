...
import { CONTENT_FAQ_PAGE_SIZE_OPTIONS } from '@/apis/contentFaqsApi'
import DataTable from '@/components/table/DataTable'
import DeleteConfirmationModal from '@/components/table/DeleteConfirmationModal'
import TablePagination from '@/components/table/TablePagination'
import { buildFaqColumns } from '@/features/content/faqs/columns'
import { faqsLabels } from '@/features/content/faqs/data'
import { useFaqsCrud } from '@/features/content/faqs/useFaqsCrud'
import EntityFormModal from '@/modules/crud/form/EntityFormModal'

const FaqsCrudTable = () => {
  const crud = useFaqsCrud({
    buildColumns: buildFaqColumns,
  })
  const deleteMessage =
    crud.pendingDeleteCount > 1
      ? `Bạn có chắc muốn xóa ${crud.pendingDeleteCount} ${faqsLabels.itemName} đã chọn?`
      : `Bạn có chắc muốn xóa ${faqsLabels.itemName} này?`
  ...
  return (
    <Card>
        ...
        <div className="d-flex align-items-center gap-2 flex-wrap">
          <div className="app-search">
            <input
              ...
              placeholder={faqsLabels.searchPlaceholder}
      ...
      {crud.isLoading ? (
        ...
        <DataTable
          table={crud.table}
          emptyMessage={faqsLabels.emptyMessage}
      ...
      {!crud.isLoading && crud.paginationInfo.total > 0 && (
        <CardFooter className="border-0">
          <TablePagination
            ...
            itemsName={faqsLabels.itemName}
            pageSize={crud.pageSize}
            pageSizeOptions={CONTENT_FAQ_PAGE_SIZE_OPTIONS}
      ...
      {crud.formMode && crud.formValues && (
        <EntityFormModal
          ...
          viewFields={crud.formConfig.viewFields}          initialValues={crud.formValues}
